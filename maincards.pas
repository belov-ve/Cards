...
begin
  ...
  begin
    ...
    Filtered  := True;  //установка курсора на последнюю обработанную запись
...
begin
  ...
  begin
    // если пачка редактируется, и кликнули по колонками Hide1 или Hid2 (видимость пачек), то меняем их соостояние на обратное
...
//надо посмотреть еще глубже на уровено в ....\Application Support\Card
  ...
  begin
    ...
    begin
      ...
      // триггер очистки статистики при изменении содержания карточки
        ...
        Add('END;');
      // представление для отбора статистики по пачкам
  ...
  try
      ...
      begin
        ...
        //
        need_upd := True; // применить обновление к карточкам
      end
      else need_upd := False; // обновление уже применялось, пропускаем его
      if DM.FDDatabese.Connected then DM.FDDatabese.Close;

  except
      need_upd := False;  // ошибка работы с файлолм обновления (или его нет) - пропускаем обновление
      if DM.FDDatabese.Connected then DM.FDDatabese.Close;
  end;


  // если необходимо проводим сравнение и обновление таблиц БД из таблиц обновения
  if DM.FDDatabese.Connected then DM.FDDatabese.Close;
  DM.FDDatabese.Params.Database := db_work;
  ...
  // Удаляю, потому что:
  //  Android - состав пакета изменять нельзя
  //  Windows - если обнолвено, повторно не будет выполнено
  //
    ...
    0:  begin
          ...
          begin
            ...
            case pack_selected^.Tag of
              1..2 :
              begin
                //btnDeletePack.Visible := True;  // кнопка удаления локальной или сетевой пачки
                mDelPack.Enabled  := True;
                mDelPack.HitTest  := True;
              end;
              else
    ...
    2:  begin
          ...
          case Card.Tag of
            ...
            0:  begin
                  ...
                  SetCardReadOnly(True);  // установка режима RedOnly
                end;
    ...
    3:  begin
          ...
          2 :   // режим обучения
            begin
              ...
              if SwitchAuto.IsChecked then
              begin
                ShowSpeedControl(True);  // включение элекменов панели выбора скорости перебора
                Timer1.TagString := DateTimeToStr( Now ); // запоминаем время начала интервала
                Timer1.Enabled := True;  // запуск авто смены карточек, если это включено
              end
              else ShowSpeedControl(False);  // отключение элекменов панели выбора скорости перебора
            end;
          end;
    // End Opros
    end;
